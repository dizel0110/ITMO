...
class Command(BaseCommand):
            ...
            if field.many_to_many:
                if field.remote_field.through and not field.remote_field.through._meta.auto_created:
                    ...
                    parser.add_argument(
                        "--%s" % field_name,
                        action="append",
                        help=("Specifies the %s for the superuser. Can be used " "multiple times." % field_name,),
                    )
            else:
                parser.add_argument(
                    "--%s" % field_name,
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.UserModel._default_manager.db_manager(database).create_akcent_superadmin(**user_data)
