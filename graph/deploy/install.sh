...
AKCENT_GRAPH_IS_RUNNING=""
if [ -f "${COMPOSE_FILE}" ]; then
  AKCENT_GRAPH_IS_RUNNING=$(docker-compose -f "${COMPOSE_FILE}" ps -q django)
fi
...
if [[ "${AKCENT_GRAPH_IS_RUNNING}" != "" ]]; then
  echo -e "${GREEN}Stopping services...${DEFAULT}"
  docker-compose -f "${COMPOSE_FILE}" down
fi

if [ -x "$(command -v s3cmd)" ]; then
  echo -e "${GREEN}Downloading GraphDB artifacts medical annoy pkls...${DEFAULT}"
  PKLS_DIR="${WORKDIR}"/private/pkls/
  mkdir -p "${PKLS_DIR}"
  s3cmd sync --no-check-md5 "${S3_PKLSDATA_URL}" "${PKLS_DIR}"

  echo -e "${GREEN}Downloading LLM tokenizer...${DEFAULT}"
  TOKENIZER_DIR="${WORKDIR}"/private/llm/tokenizer
  mkdir -p "${TOKENIZER_DIR}"
  s3cmd get --force "${LLM_TOKENIZER_URL}" "${TOKENIZER_DIR}"
else
  echo -e "${RED}Failed to download GraphDB artifacts. Please install and configure s3cmd${DEFAULT}"
fi
...
if [ ! -f "${WORKDIR}"/.env ]; then
  ...
  for key in "APPLICATION_UID" "POSTGRES_PASSWORD" "REDIS_PASSWORD" "NEO4J_PASSWORD"; do
...
if [ ! "$(grep '^AKCENT_GRAPH_HOSTNAME=' "${WORKDIR}"/.env | cut -d '=' -f2-)" ]; then
  if [[ -n ${AKCENT_GRAPH_DOMAIN+x} ]]; then
    sed -i "s|AKCENT_GRAPH_HOSTNAME=|AKCENT_GRAPH_HOSTNAME=https://${AKCENT_GRAPH_DOMAIN}|g" "${WORKDIR}"/.env
  fi
fi
...
if [ "${AKCENT_GRAPH_HOSTNAME}" ]; then
  export NGINX_HOSTNAME="${AKCENT_GRAPH_HOSTNAME##*/}"  # This variable is used in nginx.conf
...
export AKCENT_GRAPH_HOSTNAME="${AKCENT_GRAPH_HOSTNAME}"
...
# shellcheck disable=SC2143
if [ ! "$(grep '^NEO4J_PASSWORD=' "${WORKDIR}"/.env)" ]; then
  echo -e "${GREEN}Generating neo4j credentials...${DEFAULT}"
  NEO4J_PASSWORD=$(uuidgen)
  echo "NEO4J_PASSWORD=${NEO4J_PASSWORD}" >> "${WORKDIR}"/.env
fi

echo "NEO4J_BOLT_URL=bolt://${NEO4J_USER}:${NEO4J_PASSWORD}@neo4j:7687" >> "${WORKDIR}"/.env


if [ -f "${WORKDIR}"/docker-compose.yml ]; then
  cp -f "${WORKDIR}"/docker-compose.yml "${WORKDIR}"/docker-compose.yml.old
fi
cp -f "${INVOKE_DIR}"/{docker-compose.yml,.version} "${WORKDIR}"

echo -e "${GREEN}Starting services...${DEFAULT}"
for image in "django" "scheduler" "graph-celery-worker-new-med-features" "graph-celery-worker-additional-med-features" "graph-celery-worker-default" "graph-celery-worker-low"; do
  docker-compose -f "$COMPOSE_FILE" run -u root --rm --no-deps "${image}" \
    bash -c "chown -R 1001:1001 /usr/src/app/public /usr/src/app/log/ /tmp && install -d /tmp -o root -g root -m 1777"
done
...
cp -f "${INVOKE_DIR}/connect_to_sentry.sh" "${WORKDIR}"
...
echo -e "${GREEN}Akcent graph service ${PRODUCT_VERSION} successfully installed.${DEFAULT}"
