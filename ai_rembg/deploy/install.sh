...
AI_PHOTOENHANCER_IS_RUNNING=""
if [ -f "${COMPOSE_FILE}" ]; then
  AI_PHOTOENHANCER_IS_RUNNING=$(docker-compose -f "${COMPOSE_FILE}" ps -q django)
fi
...
if [[ "${AI_PHOTOENHANCER_IS_RUNNING}" != "" ]]; then
...
if [ ! "$(grep '^AI_PHOTOENHANCER_HOSTNAME=' "${WORKDIR}"/.env | cut -d '=' -f2-)" ]; then
  if [[ -n ${AI_PHOTOENHANCER_DOMAIN+x} ]]; then
    sed -i "s|AI_PHOTOENHANCER_HOSTNAME=|AI_PHOTOENHANCER_HOSTNAME=https://${AI_PHOTOENHANCER_DOMAIN}|g" "${WORKDIR}"/.env
  fi
fi
...
if [ "${AI_PHOTOENHANCER_HOSTNAME}" ]; then
  export NGINX_HOSTNAME="${AI_PHOTOENHANCER_HOSTNAME##*/}"  # This variable is used in nginx.conf
...
export AI_PHOTOENHANCER_HOSTNAME="${AI_PHOTOENHANCER_HOSTNAME}"
# export FRONT_OAUTH_PORT="${FRONT_OAUTH_PORT}"
...
for image in "django" "scheduler" "worker"; do
...
echo -e "${GREEN}AI_PhotoEnhancer ${PRODUCT_VERSION} successfully installed.${DEFAULT}"
