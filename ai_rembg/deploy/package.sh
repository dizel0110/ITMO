# The script to create AI_PhotoEnhancer package, which should be the same as built on CI. You may upload it to file storage manually.
...
if [ -z "${AI_PHOTOENHANCER_VERSION:-}" ]; then
  if [ "$#" -gt 0 ]; then
    ...
    read -rp 'Which version to build?: ' AI_PHOTOENHANCER_VERSION
  fi
fi
if [ -z "${AI_PHOTOENHANCER_VERSION}" ]; then
...
cp -r "$CURRENT_DIR/ai_photoenhancer" "$COPY_FOLDER_PATH/"
...
docker build -t ai_photoenhancer/django:latest . -f ./deploy/django/Dockerfile
docker build -t ai_photoenhancer/nginx:latest . -f ./deploy/nginx/Dockerfile
